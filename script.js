...
const moveNoButton = (e) => {
  if (e) e.preventDefault(); // Stop default click behavior
  
  // Play Sound
  try {
    noSound.currentTime = 0;
    noSound.play().catch(err => console.log("Sound needs click first"));
  } catch(e) {}
...
noBtn.addEventListener("click", moveNoButton); // Extra backup
...
function initSlideshow() {
  // Create 38 Images dynamically
  for (let i = 1; i <= TOTAL_PHOTOS; i++) {
    const img = document.createElement("img");
    img.src = `photos/${i}.jpg`; // Assumes photos/1.jpg ... photos/38.jpg
